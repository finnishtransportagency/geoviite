...
enum class BulkTransferRequestType {
    CREATE,
    EXPEDITE,
    POLL,
}
...
constructor(
    ...
    fun beginNewBulkTransfer(branch: LayoutBranch, split: PublishedSplit, timeout: Duration) {
        ...
        withBulkTransferHttpErrorHandler(split, BulkTransferRequestType.CREATE) {
    ...
    fun pollBulkTransferStateUpdate(split: PublishedSplit, timeout: Duration): PublishedSplit {
        ...
        withBulkTransferHttpErrorHandler(split, BulkTransferRequestType.POLL) {
    ...
    fun newBulkTransferCreateRequest(branch: LayoutBranch, split: PublishedSplit): RatkoBulkTransferCreateRequest {
    ...
    fun expediteBulkTransferStart(split: PublishedSplit, timeout: Duration): PublishedSplit {
        ...
        withBulkTransferHttpErrorHandler(split, BulkTransferRequestType.EXPEDITE) {
            ratkoClient.forceStartBulkTransfer(split.bulkTransfer.ratkoBulkTransferId, timeout)
            bulkTransferDao.upsert(splitId = split.id, temporaryFailure = false, state = BulkTransferState.IN_PROGRESS)
        }

        return splitService.getOrThrow(split.id) as PublishedSplit
    }

    fun withBulkTransferHttpErrorHandler(
        split: PublishedSplit,
        requestType: BulkTransferRequestType,
        httpRequest: () -> Unit,
    ) {
        try {
            httpRequest()
    ...
    fun handleTimeoutException(split: PublishedSplit) {
        if (!requireNotNull(split.bulkTransfer).temporaryFailure) {
            ...
            bulkTransferDao.upsert(splitId = split.id, temporaryFailure = true)
        } else {
    ...
    fun handleWebClientBulkTransferResponseException(split: Split, ex: WebClientResponseException) {
        ...
        when (ex.statusCode) {
            ...
            HttpStatus.GATEWAY_TIMEOUT -> {
                ...
                logger.info("Setting split bulk transfer temporary failure status to true, splitId=${split.id}")
                bulkTransferDao.upsert(splitId = split.id, temporaryFailure = true)
            }
            else -> {
                logger.info("Unhandled or fatal HTTP error, statusCode=${ex.statusCode}, splitId=${split.id}")
                logger.info("Setting split bulk transfer state to ${BulkTransferState.FAILED}, splitId=${split.id}")
                bulkTransferDao.upsert(splitId = split.id, state = BulkTransferState.FAILED)
            }
        }
    }
    ...
    fun previousRatkoPushHasNotFailed(split: PublishedSplit): Boolean {
        return try {
            ratkoPushDao.fetchPreviousPush().let { previousPush ->
                val ratkoPushFailed = previousPush.status == RatkoPushStatus.FAILED
                if (ratkoPushFailed) {
                    logger.info(
                        "Previous ratkoPushId=${previousPush.id}) has FAILED," +
                            "bulk transfer creation will be skipped for splitId=${split.id}"
                    )
                }
