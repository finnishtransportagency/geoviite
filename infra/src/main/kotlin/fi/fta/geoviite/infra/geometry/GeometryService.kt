...
constructor(
    ...
    private fun runVerticalGeometryListGeneration(op: () -> Unit) =
        runWithLock(
            verticalGeometryListingGenerationUser,
        ...
        val srid =
            geometryDao.getSwitchSrid(switchId)
                ?: throw IllegalStateException(
                    "Coordinate system not found for geometry switch $switchId!")
        ...
        return if (fileAndSource.source == PAIKANNUSPALVELU) {
            InfraModelFile(
                fileNameWithSourcePrefixIfPaikannuspalvelu(
                    fileAndSource.file.name, fileAndSource.source),
    ...
    @Transactional(readOnly = true)
    fun fetchDuplicateGeometryPlanHeader(
        newFile: InfraModelFile,
        source: PlanSource
    ): GeometryPlanHeader? {
        return geometryDao
            .fetchDuplicateGeometryPlanVersion(newFile, source)
            ?.let(geometryDao::getPlanHeader)
    }

    private fun getSwitchName(
        context: LayoutContext,
        switchId: IntId<TrackLayoutSwitch>
    ): SwitchName = switchService.get(context, switchId)?.name ?: unknownSwitchName
    ...
    fun getElementListing(
        ...
        val geocodingContext =
            plan.trackNumber?.let {
                geocodingService.getGeocodingContext(plan.trackNumber, planVersion)
            }
    ...
    fun getElementListingCsv(
        ...
        return ElementListingFile(
            FileName(
                "${translation.t("data-products.element-list.element-list-title")} ${plan.fileName}"),
            csvFileContent,
        )
    }
    ...
    fun getElementListing(
        ...
        val (track, alignment) =
            locationTrackService.getWithAlignmentOrThrow(layoutContext, trackId)
    ...
    fun getElementListingCsv(
        ...
        val elementListing =
            getElementListing(layoutContext, trackId, elementTypes, startAddress, endAddress)
        val translation = localizationService.getLocalization(lang)
        val csvFileContent = locationTrackElementListingToCsv(elementListing, translation)
        return ElementListingFile(
            FileName(
                "${translation.t("data-products.element-list.element-list-title")} ${track.name}"),
            csvFileContent)
    }
        ...
        val elementListing =
            locationTrackService
                ...
                .map { (locationTrack, alignment) ->
                    Triple(
                        locationTrack, alignment, trackNumbers[locationTrack.trackNumberId]?.number)
                }
                .sortedWith(compareBy({ (_, _, tn) -> tn }, { (track, _, _) -> track.name }))
                .flatMap { (track, alignment, trackNumber) ->
                    getElementListing(
                        track, alignment, trackNumber, geocodingContexts[track.trackNumberId])
                }
        val csvFileContent = locationTrackElementListingToCsv(elementListing, translation)
        val dateFormatter =
    ...
    fun getVerticalGeometryListingCsv(
        ...
        return FileName(
            "${translation.t("data-products.vertical-geometry.vertical-geometry-title")} ${plan.fileName}") to
            csvFileContent.toByteArray()
    }
    ...
    fun getVerticalGeometryListing(
        ...
        val (track, alignment) =
            locationTrackService.getWithAlignmentOrThrow(layoutContext, locationTrackId)
        val geocodingContext =
            geocodingService.getGeocodingContext(layoutContext, track.trackNumberId)
        return toVerticalGeometryListing(
            track,
    ...
    fun getVerticalGeometryListingCsv(
        ...
        val locationTrack =
            locationTrackService.getOrThrow(MainLayoutContext.official, locationTrackId)
        val verticalGeometryListing =
            getVerticalGeometryListing(
                MainLayoutContext.official, locationTrackId, startAddress, endAddress)
        val translation = localizationService.getLocalization(lang)

        val csvFileContent =
            locationTrackVerticalGeometryListingToCsv(verticalGeometryListing, translation)
        val name =
            FileName(
    ...
    fun makeEntireVerticalGeometryListingCsv() = runVerticalGeometryListGeneration {
        ...
        val verticalGeometryListingWithTrackNumbers =
            locationTrackService
                ...
                .sortedWith(
                    compareBy(
                        { locationTrack ->
                            geocodingContexts[locationTrack.trackNumberId]?.trackNumber
                        },
                ...
                .flatMap { locationTrack ->
                    val verticalGeometryListingWithoutTrackNumbers =
                        getVerticalGeometryListing(
                            MainLayoutContext.official, locationTrack.id as IntId)
        ...
        val translation = localizationService.getLocalization(LocalizationLanguage.FI)
        val csvFileContent =
            entireTrackNetworkVerticalGeometryListingToCsv(
                verticalGeometryListingWithTrackNumbers, translation)
        val dateFormatter =
            DateTimeFormatter.ofPattern("dd.MM.yyyy").withZone(ZoneId.of("Europe/Helsinki"))
    ...
    fun getEntireVerticalGeometryListingCsv() =
        verticalGeometryListingFileDao.getVerticalGeometryListingFile()

    private fun getHeaderAndAlignment(
        id: IntId<GeometryAlignment>
    ): Pair<GeometryPlanHeader, GeometryAlignment> {
        val header = geometryDao.fetchAlignmentPlanVersion(id).let(geometryDao::getPlanHeader)
        val geometryAlignment =
            geometryDao.fetchAlignments(header.units, geometryAlignmentId = id).first()
    ...
    private fun getComparator(
        ...
        return when (sortField) {
            ...
            GeometryPlanSortField.KM_START ->
                Comparator { a, b ->
                    nullsLastComparator(a.kmNumberRange?.min, b.kmNumberRange?.min)
                }

            GeometryPlanSortField.KM_END ->
                Comparator { a, b ->
                    nullsLastComparator(a.kmNumberRange?.max, b.kmNumberRange?.max)
                }
            GeometryPlanSortField.PLAN_PHASE ->
                stringComparator { h ->
                    h.planPhase?.let { planPhase ->
                        translation.t("enum.plan-phase.${planPhase.name}")
                    }
                }
            GeometryPlanSortField.DECISION_PHASE ->
                stringComparator { h ->
                    h.decisionPhase?.let { decisionPhase ->
                        translation.t("enum.plan-decision.${decisionPhase.name}")
                    }
                }
            GeometryPlanSortField.CREATED_AT ->
                Comparator { a, b -> nullsLastComparator(a.planTime, b.planTime) }
            ...
            GeometryPlanSortField.LINKED_AT ->
                Comparator { a, b ->
                    nullsLastComparator(
                        linkingSummaries[a.id]?.linkedAt, linkingSummaries[b.id]?.linkedAt)
                }
    ...
    private inline fun <reified T> stringComparator(crossinline getValue: (T) -> CharSequence?) =
        Comparator.comparing(
            { h: T -> getValue(h)?.toString()?.lowercase() }, ::nullsLastComparator)
    ...
    private fun collectSegmentSources(alignment: LayoutAlignment): List<SegmentSource> {
        val planAlignmentIdToPlans: MutableMap<IntId<GeometryAlignment>, RowVersion<GeometryPlan>> =
            mutableMapOf()
        ...
        return alignment.segments.map { segment ->
            ...
            if (sourceId == null || sourceId !is IndexedId || segment.sourceStart == null) {
                ...
                val planVersion =
                    planAlignmentIdToPlans.computeIfAbsent(
                        planAlignmentId, geometryDao::fetchAlignmentPlanVersion)
                val plan = geometryDao.fetchPlan(planVersion)
                val planAlignment =
                    plan.alignments.find { alignment -> alignment.id == planAlignmentId }
                        ?: return@map SegmentSource(null, null, null, plan)
                val planProfile =
                    planAlignment.profile ?: return@map SegmentSource(null, null, null, plan)
            ...
            .mapNotNull { (lastPlanEndIndex, thisPlanEndIndex) ->
                if (alignment.segments.isNotEmpty())
                    PlanLinkingSummaryItem(
                        ...
                        segmentSources[thisPlanEndIndex].alignment?.let {
                            toAlignmentHeader(null, it)
                        },
    ...
    fun getLocationTrackHeights(
        ...
        return collectTrackMeterHeights(
            ...
            val distanceInGeometryAlignment =
                distanceInElement + (source.element?.staStart?.toDouble() ?: 0.0)
    ...
    fun getPlanAlignmentHeights(
        ...
        val geometryAlignment =
            plan.alignments.find { alignment -> alignment.id == planAlignmentId } ?: return null
        ...
        return collectTrackMeterHeights(
            startDistance, endDistance, geocodingContext, alignment, tickLength) { point, _ ->
    ...
    private fun collectTrackMeterHeights(
        ...
        val addressOfStartDistance =
            geocodingContext.getAddress(alignment.getPointAtM(startDistance) ?: return null)?.first
                ?: return null
        val addressOfEndDistance =
            geocodingContext.getAddress(alignment.getPointAtM(endDistance) ?: return null)?.first
                ?: return null
            ...
            alignmentBoundaryAddresses
                .mapNotNull { boundary ->
                    alignment.getPointAtM(boundary.distanceOnAlignment)?.let { point ->
                        geocodingContext.getAddress(point)?.let { address ->
                            address.first to boundary.segmentIndex
                        }
                    }
                }
                .groupBy(
            ...
            .map { referencePointIndex ->
                ...
                val lastPoint =
                    (referencePoint.meters.toDouble() +
                            getKmLengthAtReferencePointIndex(
                                referencePointIndex, geocodingContext) - minTickSpace.toDouble())
                        .toInt()
                        .coerceAtLeast(0)
                ...
                val allTicks =
                    ((alignmentBoundaryAddressesByKm[kmNumber] ?: listOf()) +
                            ((0..lastPoint step tickLength).map { distance ->
                                distance.toBigDecimal() to null
                            }))
                    ...
                    (allTicks.filterIndexed { i, (trackMeterInKm, segmentIndex) ->
                            segmentIndex != null ||
                                ...
                                ((trackMeterInKm - allTicks[i - 1].first >= minTickSpace) &&
                                    (i == allTicks.lastIndex ||
                                        allTicks[i + 1].first - trackMeterInKm >= minTickSpace))
                        }
                        ...
                        +
                            ...
                            (if (kmNumber == alignmentEnd.address.kmNumber)
                                listOf(alignmentEnd.address.meters to null)
                            else listOf()))
                        .sortedBy { (trackMeterInKm) -> trackMeterInKm }
                    ...
                    if (kmNumber == alignmentEnd.address.kmNumber) {
                        ...
                        geocodingContext
                            .getTrackLocation(
                                alignment,
                                TrackMeter(
                                    geocodingContext.referencePoints[referencePointIndex + 1]
                                        .kmNumber,
                                    0))
                            ?.point
                            ?.m!!
    ...
    private fun getLayoutGeocodingContextForPlanTrackNumber(
        ...
        trackNumber
            ?.let { number ->
                trackNumberService.find(MainLayoutContext.official, number).firstOrNull()?.id
            }
            ?.let { trackNumberId ->
                geocodingService.getGeocodingContext(MainLayoutContext.official, trackNumberId)
            }
}
...
private fun getKmLengthAtReferencePointIndex(
    ...
    if (referencePointIndex == geocodingContext.referencePoints.size - 1) {
        geocodingContext.referenceLineGeometry.length -
