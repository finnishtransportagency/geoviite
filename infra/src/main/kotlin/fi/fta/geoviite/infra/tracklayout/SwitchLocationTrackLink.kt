    ...
    return duplicateTracksAndAlignments
        ...
        .flatMap { (duplicateTrack, duplicateAlignment) ->
            getLocationTrackDuplicatesBySplitPoints(
                mainTrack.id, mainTrackSplitPoints, duplicateTrack, duplicateAlignment)
        }
...
/**
 * This function finds duplicate-track segments, comparing the potential duplicate to the main
 * track.
 *
 * @return a pair of match start-index (for sorting matches between duplicates) and the duplicate
 *   status
 */
fun getDuplicateMatches(
    ...
    val matchIndices =
        findOrderedMatches(mainTrackSplitPoints, duplicateTrackSplitPoints) {
            splitPoint1,
            splitPoint2 ->
    ...
    return if (matchRanges.isEmpty() && duplicateOf != mainTrackId) {
        ...
        matchRanges.map { range ->
            val match =
                if (range == 0..duplicateTrackSplitPoints.lastIndex) DuplicateMatch.FULL
...
/**
 * Finds matching items between two lists, continuing the search from the previous match to never
 * seek backwards. This ordering ensures that the result indices are monotonically increasing for
 * both lists.
 *
...
fun collectSplitPoints(
    ...
    val uniqueSplitPoints =
        allSplitPoints.filterIndexed { index, splitPoint ->
            val firstIndex =
                allSplitPoints.indexOfFirst { otherSplitPoint ->
                    splitPoint.isSame(otherSplitPoint)
                }
            firstIndex == index
        }
