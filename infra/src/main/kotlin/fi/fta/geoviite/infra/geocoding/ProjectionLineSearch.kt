...
fun <M : GeocodingAlignmentM<M>> getAddressInInterval(
    ...
    while (true) {
        ...
        when (step) {
            is NotFound -> return null
            is Resolved -> return TrackMeter(step.kmNumber, round(step.meters, decimals))
            is TakeStep<M> -> {
                ...
                if (lastStepDirection != null && step.takenStepDirection != lastStepDirection) return null
                else {
...
fun <M : GeocodingAlignmentM<M>> projectionLinesHaveCrossed(
    getProjectionLinesAtIndex: (Int) -> ProjectionLine<M>?,
    point: IPoint,
    projectionInContext: ProjectionLineInContext<M>,
): Boolean {
    val left = getProjectionLinesAtIndex(projectionInContext.intervalIndex)
    val right = getProjectionLinesAtIndex(projectionInContext.intervalIndex + 1)
    return if (left == null || right == null) false
    else {
        val (leftIntersection, rightIntersection) = getQueryLineIntersections(point, left, right) ?: return false
