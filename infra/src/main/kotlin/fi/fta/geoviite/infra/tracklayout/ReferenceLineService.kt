...
class ReferenceLineService(
    ...
    private fun updatedAlignmentVersion(line: ReferenceLine) =
        ...
        // any original
        if (line.dataType == TEMP || line.isOfficial)
            alignmentService.duplicateOrNew(line.alignmentVersion)
    ...
    fun getByTrackNumberWithAlignment(
        ...
        return dao.fetchVersionByTrackNumberId(layoutContext, trackNumberId)
            ?.let(::getWithAlignmentInternal)
    }
    ...
    private fun associateWithAlignments(
        ...
        // alignmentDao.fetch
        val alignments =
            alignmentDao.fetchMany(lines.map(ReferenceLine::getAlignmentVersionOrThrow))
    ...
    override fun mergeToMainBranch(
        ...
        return mergeToMainBranchInternal(
            versions,
            line.copy(
                alignmentVersion = alignmentService.duplicate(line.getAlignmentVersionOrThrow())),
        )
    }
}
