...
//{
    ...
    //                ) args on args.id = rlv.id and args.layout_context_id = rlv.layout_context_id and args.version =
    // rlv.version
    //              left join layout.alignment_version av on rlv.alignment_id = av.id and rlv.alignment_version =
    // av.version
    ...
    //              left join layout.alignment_version av on rl.alignment_id = av.id and rl.alignment_version =
    // av.version
    //            """
    //                .trimIndent()
    //
    ...
    //            from layout.reference_line_in_layout_context(:publication_state::layout.publication_state, :design_id)
    // rl
    ...
    //                     on reference_line.alignment_id = alignment.id and reference_line.alignment_version =
    // alignment.version
    ...
    //            """
    //                .trimIndent()
    //        val params = mapOf("publication_state" to context.state.name, "design_id" to
    // context.branch.designId?.intValue)
    //        return jdbcTemplate.query(sql, params) { rs, _ ->
    //            rs.getLayoutRowVersion("id", "design_id", "draft", "version")
    //        }
    //    }
