...
interface LayoutAssetReader<T : LayoutAsset<T>> {
    ...
    fun fetchLayoutAssetChangeInfo(
    ...
    fun fetchOfficialVersionAtMomentOrThrow(
        branch: LayoutBranch,
        id: IntId<T>,
        moment: Instant
    ): LayoutRowVersion<T>

    fun fetchOfficialVersionAtMoment(
        branch: LayoutBranch,
        id: IntId<T>,
        moment: Instant
    ): LayoutRowVersion<T>?
    ...
    fun getOrThrow(context: LayoutContext, id: IntId<T>): T =
        fetch(fetchVersionOrThrow(context, id))
...
abstract class LayoutAssetDao<T : LayoutAsset<T>>(
    ...
    override fun fetchPublicationVersions(branch: LayoutBranch): List<ValidationVersion<T>> {
        return jdbcTemplate.query<ValidationVersion<T>>(
            allPublicationVersionsSql, mapOf("design_id" to branch.designId?.intValue)) { rs, _ ->
                ValidationVersion(
        ...
        return jdbcTemplate
            ...
            .also { found ->
                distinctIds.forEach { id ->
                    if (found.none { f -> f.officialId == id })
    ...
    override fun fetchLayoutAssetChangeInfo(
        layoutContext: LayoutContext,
        id: IntId<T>
    ): LayoutAssetChangeInfo? {
        return jdbcTemplate
            .query(
                ...
                mapOf(
                    "id" to id.intValue, "design_id" to layoutContext.branch.designId?.intValue)) {
                    rs,
                    _ ->
    ...
    override fun fetchVersionOrThrow(
        layoutContext: LayoutContext,
        id: IntId<T>
    ): LayoutRowVersion<T> {
        logger.daoAccess(AccessType.VERSION_FETCH, table.name, id)
    ...
    @Transactional
    override fun deleteDrafts(branch: LayoutBranch): List<LayoutDaoResponse<T>> =
        deleteDraftsInternal(branch)

    private fun deleteDraftsInternal(
        branch: LayoutBranch,
        id: IntId<T>? = null
    ): List<LayoutDaoResponse<T>> {
        val sql =
...
fun <T : LayoutAsset<T>> verifyObjectIsExisting(contextData: LayoutContextData<T>) {
    require(contextData.dataType == DataType.STORED) {
        "Cannot update TEMP row: context=$contextData"
    }
...
fun <T : LayoutAsset<T>> verifyObjectIsNew(contextData: LayoutContextData<T>) {
    require(contextData.dataType == DataType.TEMP) {
        "Cannot insert existing row as new: context=$contextData"
    }
