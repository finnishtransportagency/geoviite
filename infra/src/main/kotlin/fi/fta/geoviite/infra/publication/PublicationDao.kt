    ...
    fun fetchTrackNumberPublicationCandidates(
        ...
        val sql =
            ...
            select id, design_id, draft, version, number, change_time, change_user, design_asset_state,
              ...
              postgis.st_astext(candidate_track_number.bounding_box) as bounding_box
    ...
    fun fetchPublicationTrackNumberChanges(
        ...
        val sql =
            ...
            select
              ...
              tn.start_address as start_address,
              old_tn.start_address as old_start_address,
              tn.length as length,
              old_tn.length as old_length,
              postgis.st_x(tn_ends.start_point) as start_x,
              postgis.st_y(tn_ends.start_point) as start_y,
              postgis.st_x(tn_ends.end_point) as end_x,
              postgis.st_y(tn_ends.end_point) as end_y,
              postgis.st_x(old_tn_ends.start_point) as old_start_x,
              postgis.st_y(old_tn_ends.start_point) as old_start_y,
              postgis.st_x(old_tn_ends.end_point) as old_end_x,
              postgis.st_y(old_tn_ends.end_point) as old_end_y,
            ...
            from publication.track_number
              ...
              left join layout.track_number_version_ends_view tn_ends
                on tn_ends.id = tn.id
                  and tn_ends.layout_context_id = tn.layout_context_id
                  and tn_ends.version = tn.version
              ...
              left join layout.track_number_version_ends_view old_tn_ends
                on old_tn_ends.id = old_tn.id
                  and old_tn_ends.layout_context_id = old_tn.layout_context_id
                  and old_tn_ends.version = old_tn.version
...
// TODO: GVT-3637 cleanup
    ...
    //               where ptnk.track_number_id = rl.track_number_id and ptnk.publication_id = ptn.publication_id) as
    // changed_km
    ...
    //                          on tn.id = ptn.id and tn.layout_context_id = ptn.layout_context_id and tn.version =
    // ptn.version
    ...
    //                        version = rs.getLayoutRowVersion("id", "design_id", "draft", "version"),
    //                        baseVersion = rs.getLayoutRowVersionOrNull("id", "base_layout_context_id",
    // "base_version"),
    //                        trackNumberId = rs.getIntId("track_number_id"),
...
||||||| parent of 3d3e4b115 (GVT-3637 More test data init fixes)
    //    fun fetchPublishedReferenceLines(
    //        publicationIds: Set<IntId<Publication>>
    //    ): Map<IntId<Publication>, List<PublishedReferenceLine>> {
    //        val sql =
...
=======
>>>>>>> 3d3e4b115 (GVT-3637 More test data init fixes)
    ...
    fun fetchPublishedSwitches(
        ...
        return jdbcTemplate
            .query(sql, mapOf("publication_ids" to publicationIds.map { it.intValue })) { rs, _ ->
                ...
                val baseVersion =
                    rs.getLayoutRowVersionOrNull<LayoutSwitch>("id", "base_layout_context_id", "base_version")
