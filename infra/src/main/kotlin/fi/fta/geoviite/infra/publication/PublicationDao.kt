    ...
    fun fetchPublicationSwitchChanges(publicationId: IntId<Publication>): Map<IntId<LayoutSwitch>, SwitchChanges> {
        val sql =
            ...
            with
              ...
              touched_joint_locations as (
                select
                  joints.switch_id,
                  array_agg(track.change_side) as track_change_sides,
                  array_agg(track.location_track_id) as track_ids,
                  array_agg(ltv.track_number_id) as track_track_number_ids,
                  array_agg(ltv.name) as track_names,
                  array_agg(joints.joint_number) as track_joint_numbers,
                  array_agg(postgis.st_x(joints.location)) as track_joint_locations_x,
                  array_agg(postgis.st_y(joints.location)) as track_joint_locations_y
        ...
        return jdbcTemplate
            .query(sql, mapOf("publication_id" to publicationId.intValue)) { rs, _ ->
                ...
                val trackJointXs = rs.getListOrNull<Int>("track_joint_locations_x") ?: emptyList()
                val trackJointYs = rs.getListOrNull<Int>("track_joint_locations_y") ?: emptyList()
                val oldTrackConnections = mutableMapOf<IntId<LocationTrack>, SwitchLocationTrack>()
                val newTrackConnections = mutableMapOf<IntId<LocationTrack>, SwitchLocationTrack>()

                trackIds.forEachIndexed { index, id ->
                    val connectionMap =
                        when (requireNotNull(trackChangeSides[index])) {
                            ChangeSide.OLD -> oldTrackConnections
                            ChangeSide.NEW -> newTrackConnections
                        }
                    connectionMap.compute(id) { id, current ->
                        ...
                        val joint =
                            PublicationSwitchJoint(
                                ...
                                location =
                                    Point(
                                        requireNotNull(trackJointXs[index]).toDouble(),
                                        requireNotNull(trackJointYs[index]).toDouble(),
                                    ),
                ...
                id to
                    SwitchChanges(
                        ...
                        trackConnections =
                            Change(
                                old = oldTrackConnections.values.sortedBy { it.trackNumberId.intValue },
                                new = newTrackConnections.values.sortedBy { it.trackNumberId.intValue },
                            ),
                    )
            }
            .toMap()
