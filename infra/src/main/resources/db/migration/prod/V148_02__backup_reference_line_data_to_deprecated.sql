...
-- before the V148_03 merge migration removes them.
-- The V148_01 data fix is already applied to the source data at this point,
-- making the backup easier to use for later verification.
--
...
select
  av.id, av.version, av.bounding_box, av.segment_count, av.length, av.change_user, av.change_time, av.deleted
  from layout.alignment_version av
  where exists(
    select 1 from layout.reference_line_version rlv where rlv.alignment_id = av.id and rlv.alignment_version = av.version
  );
...
select
  s.alignment_id, s.alignment_version, s.segment_index, s.geometry_alignment_id, s.geometry_element_index,
  s.start as start_m, s.source_start as source_start_m, s.source, s.geometry_id
