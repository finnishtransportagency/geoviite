...
create temporary table edge_temp as (
  with edge_candidates as (
    select
      ...
      e.deleted,
      array_agg(
          layout.calculate_segment_hash(s.geometry_alignment_id, s.geometry_element_index, s.source_start, s.source,
                                        s.geometry_id) order by s.segment_index
      ) as segment_hashes
