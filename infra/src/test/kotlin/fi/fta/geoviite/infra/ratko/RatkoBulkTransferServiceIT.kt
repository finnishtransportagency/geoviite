...
constructor(
    ...
    fun `Bulk transfer should not be started when another bulk transfer is in progress`() {
        ...
        splitTestDataService.insertPublishedSplit().let { splitId ->
            bulkTransferDao.upsert(
                splitId = splitId,
                state = BulkTransferState.IN_PROGRESS,
    ...
    fun `Retries temporarily failed bulk transfer`() {
        ...
        statesToRetry.forEach { bulkTransferState ->
            ...
            if (bulkTransferState == BulkTransferState.PENDING) {
                bulkTransferDao.upsert(splitId = splitId, state = bulkTransferState, temporaryFailure = true)
    ...
    fun `Assigns bulk transfer FAILED status when receiving HTTP 400, 500 and others`() {
        ...
        httpStatusCodesToTest.forEach { httpStatus ->
            statesToTest.forEach { bulkTransferState ->
                ...
                bulkTransferDao.upsert(splitId, state = BulkTransferState.DONE)
            }
        }
    }
    ...
    fun `Setting a failed bulk transfer back to pending will cause it to be retried and not start another one`() {
        ...
        bulkTransferDao.upsert(splitId, state = BulkTransferState.FAILED)
        ratkoBulkTransferService.manageRatkoBulkTransfers(branch)

        // Simulates a user action.
        bulkTransferDao.upsert(splitId, state = BulkTransferState.PENDING)
    ...
    fun `Bulk transfer expedited starting works and marks the bulk transfer to be in progress`() {
        ...
        val splitId = splitTestDataService.insertPublishedGeocodableSplit()
        bulkTransferDao.upsert(splitId = splitId, expeditedStart = true)
        assertEquals(BulkTransferState.PENDING, splitDao.getPublishedSplitOrThrow(splitId).bulkTransfer.state)
    ...
    fun `Bulk transfer expedited start can be toggled`() {
        ...
        bulkTransferDao.upsert(splitId = splitId, expeditedStart = false)
        assertEquals(false, splitDao.getPublishedSplitOrThrow(splitId).bulkTransfer.expeditedStart)
    }
    ...
    fun `Bulk transfer temporary failure can be toggled`() {
        ...
        bulkTransferDao.upsert(splitId = splitId, temporaryFailure = true)
        assertEquals(true, splitDao.getPublishedSplitOrThrow(splitId).bulkTransfer.temporaryFailure)

        bulkTransferDao.upsert(splitId = splitId, temporaryFailure = false)
        assertEquals(false, splitDao.getPublishedSplitOrThrow(splitId).bulkTransfer.temporaryFailure)
    }
                ...
                .map { bulkTransferState ->
                    ...
                    when (bulkTransferState) {
                        BulkTransferState.PENDING -> {
                            bulkTransferDao.upsert(splitId = splitId, state = bulkTransferState)
                        }
