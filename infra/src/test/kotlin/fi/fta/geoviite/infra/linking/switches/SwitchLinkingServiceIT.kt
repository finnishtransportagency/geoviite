...
constructor(
    ...
    fun `linking one switch over another suggests detaching the original switch`() {
        ...
        assertEquals(
            listOf(newSwitch),
            mainDraftContext
                .fetchLocationTrackWithGeometry(throughTrack.id)!!
                .second
                .trackSwitchLinks
                .map { it.switchId }
                .distinct(),
        ...
        assertEquals(
            listOf(newSwitch),
            mainDraftContext
                .fetchLocationTrackWithGeometry(branchingTrack.id)!!
                .second
                .trackSwitchLinks
                .map { it.switchId }
                .distinct(),
    ...
    fun `linking switch over another suggests detaching the original switch, even with only split alignments`() {
        ...
        tracks.forEach { track ->
            assertEquals(
                listOf(newSwitch),
                mainDraftContext
                    .fetchLocationTrackWithGeometry(track.id)!!
                    .second
                    .trackSwitchLinks
                    .map { it.switchId }
                    .distinct(),
    ...
    fun `validateRelinkingTrack relinks okay cases and gives validation errors about bad ones`() {
        val trackNumberId =
            mainOfficialContext
                .createLayoutTrackNumber(geometry = referenceLineGeometry(segment(Point(0.0, 0.0), Point(150.0, 0.0))))
                .id
