...
constructor(
    ...
    fun `Retry failed publication`() {
        val originalTrackNumber =
            trackNumberDao.save(
                trackNumber(TrackNumber("original name"), draft = false),
                referenceLineGeometry(segment(toSegmentPoints(Point(0.0, 0.0), Point(10.0, 0.0)))),
            )
        ...
        val updatedTrackNumberVersion =
            trackNumberDao.fetch(originalTrackNumber).copy(number = TrackNumber("updated name")).let { tn ->
                trackNumberDao.save(tn, trackNumberDao.getBaseSaveParams(originalTrackNumber))
            }
