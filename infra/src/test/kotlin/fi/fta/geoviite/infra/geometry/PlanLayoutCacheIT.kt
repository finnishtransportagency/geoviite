...
@SpringBootTest
class PlanLayoutCacheIT @Autowired constructor(
    ...
    fun `plan with km posts in layout coordinates can be converted to layout`() {
        val plan = plan(
            srid = LAYOUT_SRID, kmPosts = listOf(
                GeometryKmPost(
    ...
    fun `plan with km posts in KKJ coordinates can be converted to layout`() {
        ...
        val plan = plan(
            srid = kkjSrid, alignments = listOf(
                geometryAlignment(
                    geometryLine(
                        start = kmPostKkjLocation - Point(10.0, 10.0), end = kmPostKkjLocation
                    ), geometryLine(
                        start = kmPostKkjLocation, end = kmPostKkjLocation + Point(10.0, 5.0)
                    )
                )
            ),
            kmPosts = listOf(GeometryKmPost(
                staBack = BigDecimal("1003.440894"),
                staAhead = BigDecimal("854.711894"),
                staInternal = BigDecimal("854.711894"),
                kmNumber = KmNumber(1),
