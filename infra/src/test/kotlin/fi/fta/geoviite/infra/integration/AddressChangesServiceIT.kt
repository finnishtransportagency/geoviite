...
constructor(
    ...
    val layoutTrackNumberService: LayoutTrackNumberService,
    ...
    fun addressChangesFoundWhenReferenceLineChanges() {
        ...
        moveReferenceLineGeometryPointsAndUpdate(setupData.trackNumber, setupData.referenceLineGeometry) { index, point
            ->
        ...
        val updateMoment = layoutTrackNumberDao.fetchChangeTime()
    ...
    fun createAndInsertTrackNumberAndLocationTrack(): SetupData {
        ...
        val referenceLineGeom = referenceLineGeometry(splitSegment(segment(*referenceLinePoints.toTypedArray()), 4))
        val trackNumberVersion =
            layoutTrackNumberDao.save(trackNumber(TrackNumber("TEST TN $sequence"), draft = false), referenceLineGeom)
        val trackNumber = layoutTrackNumberDao.fetch(trackNumberVersion)
        val savedReferenceLineGeometry = layoutTrackNumberDao.getBaseSaveParams(trackNumberVersion)
        val kmPost1 =
        ...
        return SetupData(locationTrack, geometry, trackNumber, savedReferenceLineGeometry, listOf(kmPost1, kmPost2))
    }
    ...
    fun moveReferenceLineGeometryPointsAndUpdate(
        ...
        layoutTrackNumberService.publish(LayoutBranch.main, version)
    }
    ...
    fun <M : AlignmentM<M>> createAddresses(vararg transitionPoints: Pair<Point, TrackMeter>): AlignmentAddresses<M> {
        val addressPoints = transitionPoints.flatMapIndexed { index, transitionPoint ->
            ...
            if (nextTransitionPoint != null) {
                ...
                val addressPoints = alignmentPoints.map { alignmentPoint ->
