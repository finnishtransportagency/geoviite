...
import {
    initialLocationTrackNameBetweenOperatingPoints,
    initialLocationTrackNameChord,
    initialLocationTrackNameUndefined,
...
export const splitReducers = {
    onStartSplitting: (state: TrackLayoutState, { payload }: PayloadAction<SplitStart>): void => {
        ...
        if (locationTrackSelected) {
            ...
            const nameAndDesc =
                duplicateAtStart?.namingScheme === undefined
                    ? ({
                          ...getNameForTarget(startSwitch, endSwitch),
                          hasAutogeneratedName: true,
                      } as const)
                    : ({
                          namingScheme: duplicateAtStart.namingScheme,
                          descriptionBase: '',
...
function getLocationTrackName(
    startSwitchName: string | undefined,
    endSwitchName: string | undefined,
): LocationTrackNaming {
    const [startOperatingPoint, _startSwitch] = splitSwitchName(startSwitchName);
    const [endOperatingPoint, _endSwitch] = splitSwitchName(endSwitchName);

    if (startSwitchName === undefined || endSwitchName === undefined) {
        return initialLocationTrackNameUndefined;
    } else if (startOperatingPoint === endOperatingPoint) {
...
function splitPointToSplitTargetCandidate(
    ...
    const namingScheme = duplicateAt?.namingScheme || initialLocationTrackNameUndefined;
    ...
    return {
        ...
        focusBehaviour: splitFocusBehaviour,
        hasAutogeneratedName: namingScheme === initialLocationTrackNameUndefined,
