        ...
        if (locationTrackSelected) {
            ...
            const nameAndDesc =
                duplicateAtStart?.name === undefined
                    ? ({
                          ...getNameForTarget(
                              payload.locationTrack.nameStructure,
                              payload.trackNumber,
                              startSwitch,
                              endSwitch,
                          ),
                    ...
                    : ({
                          namingScheme: duplicateAtStart.nameStructure.namingScheme,
                          nameFreeText:
                              duplicateAtStart?.nameStructure &&
                              'nameFreeText' in duplicateAtStart.nameStructure
                                  ? duplicateAtStart?.nameStructure?.nameFreeText
                                  : '',
                          nameSpecifier:
                              duplicateAtStart?.nameStructure &&
                              'nameSpecifier' in duplicateAtStart.nameStructure
                                  ? duplicateAtStart?.nameStructure?.nameSpecifier
                                  : undefined,
...
function splitPointToSplitTargetCandidate(
    ...
    return {
        ...
        splitPoint: splitPoint,
        namingScheme:
            duplicateAt?.nameStructure?.namingScheme ?? LocationTrackNamingScheme.FREE_TEXT,
        nameFreeText:
            duplicateAt?.nameStructure && 'nameFreeText' in duplicateAt.nameStructure
                ? duplicateAt.nameStructure.nameFreeText
                : undefined,
        nameSpecifier:
            duplicateAt?.nameStructure && 'nameSpecifier' in duplicateAt.nameStructure
                ? duplicateAt.nameStructure.nameSpecifier
                : undefined,
        name,
...
function getNameForTarget(
    _mainTrackName: TrackNameStructure,
    trackNumber: TrackNumber,
    ...
    const nameFreeText =
        startSwitch?.parsedName !== undefined && endSwitch?.parsedName !== undefined
            ? `${startSwitch.parsedName.shortNumberPart}-${endSwitch.parsedName.shortNumberPart}`
            : '';
...
function updateAutogeneratedSplitNamesFromState(splitState: SplittingState): {
    ...
    return updateAutogeneratedSplitNames(
        splitState.originLocationTrack.nameStructure,
...
function updateAutogeneratedSplitNames(
    mainTrackName: TrackNameStructure,
    trackNumber: TrackNumber,
    ...
    const newFirstSplit =
        firstSplit.hasAutogeneratedName && splits.length > 0
            ? {
                  ...firstSplit,
                  ...getNameForTarget(mainTrackName, trackNumber, firstSwitch, firstSplitEndSwitch),
              }
            : firstSplit;
    ...
    const newSplits = splits.map((split, index) => {
        ...
        return split.hasAutogeneratedName
            ? {
                  ...split,
                  ...getNameForTarget(mainTrackName, trackNumber, splitStartSwitch, splitEndSwitch),
              }
            : split;
    });
