    ...
    return duplicates.find((duplicate) => {
        ...
        switch (splitPoint.type) {
            case 'switchSplitPoint':
                return (
                    duplicateSplitPoint.type == 'switchSplitPoint' &&
                    duplicateSplitPoint.switchId == splitPoint.switchId
                );
            case 'endpointSplitPoint':
                return (
                    duplicateSplitPoint.type == 'endpointSplitPoint' &&
...
export function getAllowedSwitchesFromState(state: SplittingState) {
    const startSwitchId =
        state.startSplitPoint.type == 'switchSplitPoint' && state.startSplitPoint.switchId;
    const endSwitchId =
        state.endSplitPoint.type == 'switchSplitPoint' && state.endSplitPoint.switchId;
...
export const splitReducers = {
    onStartSplitting: (state: TrackLayoutState, { payload }: PayloadAction<SplitStart>): void => {
        ...
        if (locationTrackSelected) {
            const startSwitch = payload.trackSwitches.find(
                (sw) =>
                    payload.startSplitPoint.type == 'switchSplitPoint' &&
                    sw.switchId == payload.startSplitPoint.switchId,
            );
            const endSwitch = payload.trackSwitches.find(
                (sw) =>
                    payload.endSplitPoint.type == 'switchSplitPoint' &&
    ...
    addSplit: (
        ...
        addSplitToState(state, splitPoint);
    },
    markSplitOld: (
        state: TrackLayoutState,
        { payload: splitPoint }: PayloadAction<SplitPoint>,
...
function addSplitToState(state: TrackLayoutState, splitPoint: SplitPoint) {
    ...
    const allowedSwitchIndex = allowedSwitches?.findIndex(
        (sw) => splitPoint.type == 'switchSplitPoint' && sw.switchId == splitPoint.switchId,
    );
    const isAlreadySplit = state.splittingState?.splits.some((split) =>
    ...
    if (
        ...
        const duplicateAt = findDuplicateStartingAt(
...
function updateAutogeneratedSplitNames(
    ...
    const firstSwitch = trackSwitches.find(
        (sw) =>
            startSplitPoint?.type == 'switchSplitPoint' && sw.switchId == startSplitPoint.switchId,
    );
    const firstSplitEndSplitPoint = splits.length > 0 ? expectDefined(splits[0]).splitPoint : endSplitPoint;
    const firstSplitEndSwitch = trackSwitches.find(
        (sw) =>
            firstSplitEndSplitPoint?.type == 'switchSplitPoint' &&
    ...
    const endSwitch = trackSwitches.find(
        (sw) => endSplitPoint?.type == 'switchSplitPoint' && sw.switchId == endSplitPoint.switchId,
    );
    const newSplits = splits.map((split, index) => {
        ...
        const splitEndSwitch = nextSplit
            ? trackSwitches.find(
                  (sw) =>
                      nextSplit.splitPoint.type == 'switchSplitPoint' &&
