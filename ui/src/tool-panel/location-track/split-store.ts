...
function getSwitchById(
    ...
    const sw = switches.find((switchToCheck) => switchToCheck.switchId === switchId);
    if (sw == undefined) {
        throw new Error(`Switch "${switchId}" does not exists in track switches`);
    }
    return sw;
}
...
export const splitReducers = {
    onStartSplitting: (state: TrackLayoutState, { payload }: PayloadAction<SplitStart>): void => {
        ...
        if (locationTrackSelected) {
            const startSwitch = getSwitchById(
    ...
    removeSplit: (state: TrackLayoutState, { payload }: PayloadAction<LayoutSwitchId>): void => {
        if (state.splittingState) {
            ...
            state.splittingState = {
                ...
                highlightedSwitch:
                    payload == state.splittingState.highlightedSwitch
                        ? undefined
...
function addSplitToState(state: TrackLayoutState, switchId: LayoutSwitchId) {
    ...
    const isAlreadySplit = state.splittingState?.splits.some((s) => s.switch.switchId === switchId);
    const allowedSwitch =
    ...
    if (
        ...
        !state.splittingState.splits.some((split) => split.switch.switchId === switchId)
    ) {
