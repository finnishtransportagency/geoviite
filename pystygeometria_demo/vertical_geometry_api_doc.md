...
| rataosoite                | track address                        | Format: `0623+0931.105` — kilometer number before the `+`, meter distance within that kilometer after it. Track kilometers have arbitrary lengths: they are defined by km posts, which were generally initially placed a physical kilometer apart along the reference line but the reference line's shape may change over time. |
